...
ifeq ($(config),release_x32)
  ...
  TARGET = $(TARGETDIR)/libgmcl_midi.so
  OBJDIR = obj/x32
  DEFINES += -DNDEBUG -DGMMODULE -D__LINUX_ALSA__
  ...
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib32 -m32 -shared -Wl,-soname=libgmcl_midi.so
...
ifeq ($(config),release_x64)
  ...
  LIBS += -lpthread -lasound
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -shared -Wl,-soname=libgmcl_midi.so
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
  endef
  define PRELINKCMDS
